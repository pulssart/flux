...
export async function POST(req: NextRequest) {
  try {
    ...
    const user = lang === "fr"
      ? `Titre: ${safeTitle}\nRésumé: ${safeSummary}\nContexte (ne pas inclure dans le post): ${safeUrl}\n\nContraintes globales (tous styles):\n- Évite les mots clichés (ex: "plonger dans", "révolutionnaire", "innombrables"). Préfère simple et précis.\n- Pas de politesse artificielle ("cher lecteur", "nous vous invitons humblement"). Reste direct et naturel.\n- Coupe les phrases trop longues en 2–3. Supprime le remplissage ("en effet, il est important de noter que…").\n- Varie le rythme: alterne phrases courtes et un peu plus longues.\n- Imperfections humaines: ajoute parfois une hésitation ou une opinion perso ("franchement", "à titre perso").\n- Évite le jargon académique ("paradigme", "holistique"). Préfère concret ("approche", "vue d'ensemble").\n- Évite les transitions génériques répétées ("en conclusion", "par ailleurs"). Varie ou coupe.\n- Aucune hallucination: n'invente jamais chiffres, lieux, citations. Reste fidèle au contenu fourni.\n- Ton pas trop impersonnel: quand pertinent, ancre avec une micro‑anecdote ou un ressenti ("ça m'est arrivé…").\n- Pas d'analogies hasardeuses (ex: "c'est un peu comme un sprint").\n- Semer à l'occasion 1–2 mots d'argot courant si ça sonne naturel.\n\nAutres contraintes: 1) Respecte le style choisi 2) AUCUN lien 3) AUCUN emoji 4) Pas de hashtags inutiles 5) Français courant, lisible, lignes courtes 6) Ne pose AUCUNE question 7) Introduis 1–2 micro‑fautes naturelles très subtiles.`
      : `Title: ${safeTitle}\nSummary: ${safeSummary}\nContext (do not include in post): ${safeUrl}\n\nGlobal rules (all styles):\n- Avoid clichés ("dive into", "revolutionary", "countless"). Prefer simple, precise wording.\n- No artificial politeness ("dear reader", "we humbly invite you"). Keep it direct and natural.\n- Split overly long sentences into 2–3; remove filler ("indeed, it is important to note that…").\n- Vary rhythm: alternate short and slightly longer sentences.\n- Human touch: occasional hesitation or personal take ("honestly", "personally").\n- Avoid academic jargon ("paradigm", "holistic"). Prefer concrete terms ("approach", "overview").\n- Avoid generic transitions repeated ("in conclusion", "moreover"). Vary or cut.\n- No hallucinations: never invent numbers, places, quotes. Stay faithful to provided content.\n- Avoid too impersonal tone: if relevant, add a tiny anecdote or feeling.\n- Avoid strained analogies (e.g., "it's like a sprint").\n- Occasionally sprinkle 1–2 common slang words if it feels natural.\n\nOther constraints: 1) Respect the chosen style 2) NO link 3) NO emojis 4) No unnecessary hashtags 5) Plain, easy English with short lines 6) Do NOT ask any question 7) Add 1–2 tiny, subtle natural mistakes for a human feel.`;
